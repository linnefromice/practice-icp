...
thread_local! {
    static STATE_CELL: RefCell<String> = RefCell::new(String::new());
    static STATE_VEC: RefCell<Vec<String>> = RefCell::new(Vec::new());
...
#[ic_cdk::query]
#[candid::candid_method(query)]
fn get_state_cell() -> (String, u64) {
    let start = ic_cdk::api::instruction_counter();
    let res = STATE_CELL.with(|cell| cell.borrow().clone());
    (res, ic_cdk::api::instruction_counter() - start)
}

#[ic_cdk::update]
#[candid::candid_method(update)]
fn set_state_cell(val: String) -> u64 {
    let start = ic_cdk::api::instruction_counter();
    STATE_CELL.with(|cell| cell.borrow_mut().push_str(&val));
    ic_cdk::api::instruction_counter() - start
}

#[ic_cdk::query]
#[candid::candid_method(query)]
fn get_state_vec(idx: u64) -> (Option<String>, u64) {
    let start = ic_cdk::api::instruction_counter();
    let res = STATE_VEC.with(|vec| vec.borrow().get(idx as usize).cloned());
    let count = ic_cdk::api::instruction_counter() - start;
    (res, count)
}

#[ic_cdk::update]
#[candid::candid_method(update)]
fn insert_to_state_vec(val: String) -> u64 {
    let start = ic_cdk::api::instruction_counter();
    STATE_VEC.with(|vec| vec.borrow_mut().push(val));
    ic_cdk::api::instruction_counter() - start
}

#[ic_cdk::query]
#[candid::candid_method(query)]
fn get_tuple_cell() -> (SnapshotTuple, u64) {
    let start = ic_cdk::api::instruction_counter();
    let res = TUPLE_CELL.with(|cell| cell.borrow().get().clone());
    (res, ic_cdk::api::instruction_counter() - start)
}

#[ic_cdk::update]
#[candid::candid_method(update)]
fn set_tuple_cell(val: SnapshotTuple) -> Result<u64, (String, u64)> {
    let start = ic_cdk::api::instruction_counter();
    let res = TUPLE_CELL.with(|cell| cell.borrow_mut().set(val));
    let count = ic_cdk::api::instruction_counter() - start;
    res.map(|_| (count))
        .map_err(|e| (format!("{:?}", e), count))
}

#[ic_cdk::query]
#[candid::candid_method(query)]
fn get_tuple_vec(idx: u64) -> (Option<SnapshotTuple>, u64) {
    let start = ic_cdk::api::instruction_counter();
    let res = TUPLE_VEC.with(|vec| vec.borrow().get(idx));
    let count = ic_cdk::api::instruction_counter() - start;
    (res, count)
}
...
#[ic_cdk::update]
#[candid::candid_method(update)]
fn insert_to_tuple_vec(val: SnapshotTuple) -> Result<u64, (String, u64)> {
    let start = ic_cdk::api::instruction_counter();
...
#[ic_cdk::query]
#[candid::candid_method(query)]
fn get_struct_cell() -> (SnapshotStruct, u64) {
    let start = ic_cdk::api::instruction_counter();
    let res = STRUCT_CELL.with(|cell| cell.borrow().get().clone());
    let count = ic_cdk::api::instruction_counter() - start;
    (res, count)
}
...
#[ic_cdk::update]
#[candid::candid_method(update)]
fn set_struct_cell(val: SnapshotStruct) -> Result<u64, (String, u64)> {
    let start = ic_cdk::api::instruction_counter();
...
#[ic_cdk::query]
#[candid::candid_method(query)]
fn get_struct_vec(idx: u64) -> (Option<SnapshotStruct>, u64) {
    let start = ic_cdk::api::instruction_counter();
    let res = STRUCT_VEC.with(|vec| vec.borrow().get(idx));
    let count = ic_cdk::api::instruction_counter() - start;
    (res, count)
}
...
#[ic_cdk::update]
#[candid::candid_method(update)]
fn insert_to_struct_vec(val: SnapshotStruct) -> Result<u64, (String, u64)> {
    let start = ic_cdk::api::instruction_counter();
