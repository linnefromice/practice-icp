...
shared({ caller = initializer }) actor class() {
  ...
  public shared({ caller }) func assign_role(assignee: Principal, new_role: ?Role) : async () {
    ...
    switch new_role {
      case (?#owner) throw Error.reject("Cannot assign anyone to be the owner");
      case (_) {};
    };
    if (assignee == initializer) {
      throw Error.reject("Cannot assign a new role to the canister owner");
    };
  ...
  public shared({ caller }) func caller_principal() : async Principal {
    return caller;
  };
